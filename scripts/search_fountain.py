#!/usr/bin/env python3
"""
Search for a specific fountain in the downloaded JSON file
"""

import json
import math

def search_fountain_in_file():
    """Search for the specific fountain in the downloaded data"""
    ...
    try:
        with open('./italy_data/fountains_firebase_20250817_003340.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
        print(f"✅ Loaded {len(data)} fountains from downloaded file")
    ...
    if not found_fountains:
        ...
        print(f"\n✅ Found {len(found_fountains)} exact match(es) in downloaded data!")
