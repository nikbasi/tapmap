#!/usr/bin/env python3
"""
Simplified Italian Fountains Import Script
Imports the downloaded Italian fountains into Firestore database.
This version works with the existing Firebase configuration.
"""
...
class SimpleItalyFountainsImporter:
    """Imports Italian fountains into Firestore database using existing config"""
    ...
    def initialize_firebase(self):
        """Initialize Firebase connection using existing config"""
        try:
            # Try to initialize with existing app if it exists
            try:
                app = firebase_admin.get_app()
                logger.info("Using existing Firebase app")
            except ValueError:
                # Create new app with default config
    ...
    def import_fountains(self, fountains_data: dict, batch_size: int = 100) -> int:
        """
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=100, 
                       help='Batch size for database operations (smaller for testing)')
    ...
    try:
        ...
        importer = SimpleItalyFountainsImporter()
